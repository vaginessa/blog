...
func newNote(lines []string) *note {
	...
	rawBody, tags := buildBodyFromLines(lines)
	body := noteToHTML(rawBody)
...
func writeUpdatedNotes(path string, notesPerDay []*notesForDay) {
	var buf bytes.Buffer
	for _, day := range notesPerDay {
		buf.WriteString(day.DayStr + "\n")
		notes := day.Notes
		lastNoteIdx := len(notes) - 1
		for idx, note := range notes {
			buf.WriteString("Id: " + note.ID + "\n")
			if note.Title != "" {
				buf.WriteString("Title: " + note.Title + "\n")
			}
			tags := ""
			...
			buf.WriteString("\n" + note.rawBody + "\n")
			if len(notes) > 1 && idx != lastNoteIdx {
				buf.WriteString("\n---\n\n")
			}
		}
		buf.WriteString("\n")
...
func readNotes(path string) error {
	...
	if didUpdateNotesID {
		writeUpdatedNotes("articles/updated-notes.txt", notesDays)
		fmt.Printf("\n\nHad to generate note id. This updated notes.txt.\nCheckin updated notes.\n")
		os.Exit(1)
	}
