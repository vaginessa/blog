...
Format: Textile
--------------
After we "took care of compacting s3 logs":/article/Compacting-s3-aws-logs.html, it's time to parse them. s3 log format is "well documented":http://docs.amazonwebservices.com/AmazonS3/latest/index.html?LogFormat.html and can be parsed with a simple regular expression. 

It took me some time to craft it, so here it is. You can also view the source "here":http://code.google.com/p/kjk/source/browse/trunk/scripts/test_parse_s3_log.py
...
This snippet only shows how to break one s3 log line into its components (@parse_s3_log_line@). Some work is needed to build upon this for parsing log files and extracting useful information out of them. For doing that, I recommend techniques described in "Generator Tricks for Systems Programmers":http://www.dabeaz.com/generators-uk/
